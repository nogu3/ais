...
**検証済みファームウェア: `Ver.2.97M-03`**。このバージョンの実機で全サブコマンド（読み取り・照明制御・exit code 規約）の動作を確認しており、解釈層の契約はこれを基準に凍結する。

> フィクスチャ（`tests/fixtures/`）は公開情報から再構成したサニタイズ済みのもの。実機から取得したサニタイズ済み HTML への差し替えを推奨（手順は後述）。
...
> 本日分の総計・回路別・`--date` の日付指定は実機（`Ver.2.97M-03`）で検証済み。月次積算はエンドポイント未確認のため未対応。
...
| 種別 | 方式 | 検証状況 |
|---|---|---|
| 照明（リンクプラス） | `device:{onoff: 0x30/0x31}`（希望状態を送る） | 実機（`Ver.2.97M-03`）で on / off / 状態反映を検証済み |
